...
decryptAccountKeyPair ::
    Password ->
    -- | The key index that belongs to the key pair, only used in error message.
    ID.KeyIndex ->
    EncryptedAccountKeyPair ->
    -- | The decrypted 'AccountKeyPair' or an error message on failure.
    ExceptT String IO AccountKeyPair
decryptAccountKeyPair pwd keyIndex EncryptedAccountKeyPairEd25519{..} = do
    signKey <-
        decryptJSON encryptedSignKey pwd
            `embedErr` ((("cannot decrypt signing key with index " ++ show keyIndex ++ ": ") ++) . displayException)
