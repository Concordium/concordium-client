...
import GHC.Integer (modInteger, remInteger)
...
getJSONUsingSchema typ = case typ of
  ...
  where
    ...
    getFieldsAsJSON fields = case fields of
      ...
      where
        getPair (k, v) = (k,) <$> getJSONUsingSchema v
    ...
    timestampToRFC3339 millis = Text.pack . iso8601Show $ utcTime
      where
        utcTime = addUTCTime diffTime (posixSecondsToUTCTime 0)
...
putJSONUsingSchema typ json = case (typ, json) of
    ...
    (Array expectedLen elemType, AE.Array vec) -> do
        let ls = V.toList vec
            actualLen = length ls
        unless (actualLen == fromIntegral expectedLen) $
            addTraceInfo $
  ...
  where
    ...
    fromScientific x numType =
        if isFloating x
            then Left [i|#{x} is a float, but it should have been of type #{numType}.|]
    ...
    lookupAndPut ::
        [(Text, SchemaType)] -> -- \^ The names and types for Named Fields.
    ...
    rfc3339ToTimestamp s = case iso8601ParseM timeString of
      ...
      where
        utcTimeToTimestamp :: Time.UTCTime -> Either String Word64
        utcTimeToTimestamp t
          ...
          where
            frac = 1000 * toRational (diffUTCTime t (posixSecondsToUTCTime 0)) -- conversion functions give seconds
