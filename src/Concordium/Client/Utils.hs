...
energyFromStringInform s =
    ...
    case readMaybe s :: Maybe Integer of
        Just a ->
            if a >= nrgMinBound && a <= nrgMaxBound
                then Right . fromIntegral $ a
                else Left errMsg
        Nothing -> Left errMsg
  where
    errMsg = [i|Invalid energy '#{s}'. Energy must be an integer between #{nrgMinBound} and #{nrgMaxBound}, both inclusive.|]
    nrgMinBound = 0
...
aliasFromStringInform s =
    ...
    case readMaybe s :: Maybe Integer of
        Just a ->
            if a >= aliasMinBound && a <= aliasMaxBound
                then Right . fromIntegral $ a
                else Left errMsg
        Nothing -> Left errMsg
  where
    errMsg = [i|Invalid alias number '#{s}'. Alias must be an integer between #{aliasMinBound} and #{aliasMaxBound}, both inclusive.|]
    aliasMinBound = 0
...
thresholdFromStringInform s =
    case readMaybe s :: Maybe Integer of
        ...
        Nothing -> Left errString
  where
    errString = "Invalid signature threshold. A signature threshold must be an integer between 1 and 255 inclusive."
...
accountThresholdFromStringInform s =
    case readMaybe s :: Maybe Integer of
        ...
        Nothing -> Left errString
  where
    errString = "Invalid account threshold. A signature threshold must be an integer between 1 and 255 inclusive."
...
credentialIndexFromStringInform s =
    case readMaybe s :: Maybe Integer of
        Just a
            | a >= 0 && a <= 255 -> Right (IDTypes.CredentialIndex (fromIntegral a))
...
textToDuration t = mapM measureToMs measures >>= Right . sum
  where
    ...
    measureToMs m = do
        ...
        Right $ value * unit'
      where
        invalidMeasureErrorMsg =
            [iii|"Invalid measure of time '#{m}'.
        ...
        word64FromString s = case readMaybe s :: Maybe Integer of
            ...
            Just x ->
                if x >= word64MinBound && x <= word64MaxBound
