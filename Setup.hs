...
autoConfHook desc flags = do
  case buildOS of
    Linux -> let extraFlags =
                  if lookupFlagAssignment (mkFlagName "static") (configConfigurationsFlags flags) /= Just True
                  -- embed RPATH so that the binaries can be run without setting library paths directly.
                  ...
                  then flags { configProgramArgs = ("ghc", map ("-optl-Wl,-rpath," ++) (configExtraLibDirs flags)): configProgramArgs flags}
                  else flags
